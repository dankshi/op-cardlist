...
export default function DeckViewPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <Link href="/decks" className="text-sm text-zinc-400 light:text-gray-500 hover:text-zinc-200 light:hover:text-gray-700 mb-4 inline-block">
        &larr; Back to Decks
      </Link>

      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-zinc-100 light:text-gray-900">{deck.name}</h1>
          {deck.user && (
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-zinc-400 light:text-gray-500">{deck.view_count} views</span>
        </div>
      </div>

      {deck.description && (
        <p className="text-zinc-400 light:text-gray-500 mb-6">{deck.description}</p>
      )}
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-zinc-900 light:bg-white border border-zinc-800 light:border-gray-200 rounded-lg p-3 text-center">
          <p className="text-xl font-bold text-zinc-100 light:text-gray-900">{totalCards}</p>
          <p className="text-xs text-zinc-400 light:text-gray-500">Cards</p>
        </div>
        <div className="bg-zinc-900 light:bg-white border border-zinc-800 light:border-gray-200 rounded-lg p-3 text-center">
          <p className="text-xl font-bold text-zinc-100 light:text-gray-900">{mainDeck.length}</p>
          <p className="text-xs text-zinc-400 light:text-gray-500">Unique</p>
        </div>
        <div className="bg-zinc-900 light:bg-white border border-zinc-800 light:border-gray-200 rounded-lg p-3 text-center">
          <p className="text-xl font-bold text-zinc-100 light:text-gray-900">{sideboard.length}</p>
          <p className="text-xs text-zinc-400 light:text-gray-500">Sideboard</p>
        </div>
      </div>
      ...
      {mainDeck.length > 0 && (
        <div className="mb-6">
          <h2 className="text-lg font-bold text-zinc-100 light:text-gray-900 mb-3">Main Deck ({mainDeck.reduce((s, c) => s + c.quantity, 0)})</h2>
      ...
      {sideboard.length > 0 && (
        <div className="mb-6">
          <h2 className="text-lg font-bold text-zinc-100 light:text-gray-900 mb-3">Sideboard ({sideboard.reduce((s, c) => s + c.quantity, 0)})</h2>
          <div className="space-y-1">
            {sideboard.map(card => (
              <div key={card.id} className="flex items-center justify-between px-3 py-2 rounded bg-zinc-900 light:bg-white border border-zinc-800 light:border-gray-200">
                <div className="flex items-center gap-2">
      ...
      {cards.length === 0 && (
        <div className="text-center py-12">
          <p className="text-zinc-400 light:text-gray-500">This deck has no cards yet.</p>
        </div>
      )}
      ...
      {isOwner && (
        <div className="flex gap-3 mt-8">
          <button
            ...
            className="px-4 py-2 rounded-lg border border-zinc-700 light:border-gray-300 text-zinc-300 light:text-gray-600 hover:bg-zinc-800 light:hover:bg-gray-50 text-sm transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg border border-red-800 light:border-red-300 text-red-400 hover:bg-red-500/10 light:hover:bg-red-50 text-sm transition-colors cursor-pointer"
          >
            Delete Deck
          </button>
        </div>
