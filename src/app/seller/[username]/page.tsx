...
export default async function SellerStorefrontPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  return (
    <div>
      {/* Seller header */}
      <div className="bg-zinc-900 light:bg-white border border-zinc-800 light:border-gray-200 rounded-2xl p-8 mb-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-zinc-100 light:text-gray-900">{profile.display_name}</h1>
            ...
            <div className="flex items-center gap-4 mt-2 text-sm">
              ...
              <span className="text-zinc-500 light:text-gray-400">{profile.total_sales} sales</span>
              <span className="text-zinc-500 light:text-gray-400">
                Member since {new Date(profile.created_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
              </span>
            </div>
          </div>
        </div>
        {profile.bio && <p className="text-zinc-300 light:text-gray-600 mt-4">{profile.bio}</p>}
      </div>

      {/* Listings */}
      <h2 className="text-xl font-semibold text-zinc-100 light:text-gray-900 mb-4">
        Cards for Sale ({(listings as Listing[])?.length || 0})
      </h2>

      {!listings?.length ? (
        <p className="text-zinc-500 light:text-gray-400 text-center py-8">This seller has no active listings.</p>
      ) : (
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {listing.quantity_available > 1 && (
                    <span className="text-xs text-zinc-500 light:text-gray-400">{listing.quantity_available} available</span>
                  )}
                </div>
              </div>
      ...
      {reviews && reviews.length > 0 && (
        <div className="mt-12">
          <h2 className="text-xl font-semibold text-zinc-100 light:text-gray-900 mb-4">Recent Reviews</h2>
          <div className="space-y-3">
            {reviews.map((review: { id: string; rating: number; comment: string | null; created_at: string; reviewer: { display_name: string } | null }) => (
              <div key={review.id} className="p-4 rounded-lg bg-zinc-900 light:bg-white border border-zinc-800 light:border-gray-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-yellow-400">{'★'.repeat(review.rating)}{'☆'.repeat(5 - review.rating)}</span>
                  <span className="text-xs text-zinc-500 light:text-gray-400">{new Date(review.created_at).toLocaleDateString()}</span>
                </div>
                {review.comment && <p className="text-zinc-300 light:text-gray-600 text-sm">{review.comment}</p>}
                <p className="text-xs text-zinc-500 light:text-gray-400 mt-2">&mdash; {review.reviewer?.display_name || 'Anonymous'}</p>
              </div>
            ))}
          </div>
        </div>
