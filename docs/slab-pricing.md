...
1. **Managed scraping vendor (recommended primary).** Route eBay sold-search requests through a service that owns the residential-proxy + CAPTCHA + HTML-drift maintenance (Apify's eBay actor, Zyte, Bright Data, Oxylabs, ScraperAPI). They return structured results or raw HTML; our existing `parseGradeFromTitle` / `parsePrice` / `parseEbaySoldDate` in [scrape-ebay-graded.ts](../scripts/scrape-ebay-graded.ts) still do the title→`slab_sales` normalization. Modest monthly cost, justified for a core data asset.
...
3. **Multi-source + hardened eBay.** Alt-native-auction adapter + cross-source dedup pass + the ingestion worker. Because eBay has no API and Actions IPs are blocked, this phase moves eBay off Actions onto a residential-IP path (managed vendor or self-hosted proxy worker — see [eBay ingestion without an API](#ebay-ingestion-without-an-api)). `update-slab-prices.yml` orchestrates the API-friendly sources + the recompute; eBay runs on the worker.
...
- **Portfolio time-series still values graded holdings at raw prices.** `getPortfolioValueSeries` in [collection-history.ts](../src/lib/collection-history.ts) is product/raw-keyed and has no historical comp data to draw on — `slab_market_values` is a *current* snapshot, not history. Phase 1 makes the point-in-time portfolio **total** comp-correct (via `holdingMarketPrice` in [collection.ts](../src/lib/collection.ts)), but the chart line over time is still raw-only, so a slab-heavy portfolio's chart endpoint won't match its headline total. Fixing this needs a daily snapshot table (a graded analog of `tcgplayer_card_price_history`, appended by `compute-slab-values.ts`) — deferred to a later phase.
